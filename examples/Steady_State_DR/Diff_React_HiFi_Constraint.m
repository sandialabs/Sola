classdef Diff_React_HiFi_Constraint < Constraint
    ...
    methods (Access = public)
        ...
        function [R,R_u] = Assemble_Reaction_Function(this,u)
           ...
           x_nodes = this.nodes_to_coll_points*this.x;
           [R_nodes,R_prime_nodes] = this.Reaction_Function(u_nodes,x_nodes);
        ...
        function [R_uu] = Assemble_Reaction_Function_Hessian(this,u,lambda)
            ...
            x_nodes = this.nodes_to_coll_points*this.x;
            R_uu = this.nodes_to_coll_points'*diag(this.coll_weights)*this.Reaction_Function_Hessian(u_nodes,lambda_nodes,x_nodes)*this.nodes_to_coll_points;
        end
        
        function [R,R_prime] = Reaction_Function(this,u,x)
           w = this.Weight_Function(x);
           R = w.*(u.^2);
           R_prime = 2*diag(u.*w);
        end
        ...
        function [R_prime_prime] = Reaction_Function_Hessian(this,u,lambda,x)
            w = this.Weight_Function(x);
            R_prime_prime = 2*diag(lambda.*w);
        end
        
        function [w] = Weight_Function(this,x)
            w = 1+.7*sin(2*pi*x);
        end
    ...
    methods (Access = public)
        ...
        function this = Diff_React_HiFi_Constraint(diff_react_con)
            this = this@Constraint();
            this.diff_react = diff_react_con;
            this.m = diff_react_con.m;
            this.diff_coeff = diff_react_con.diff_coeff;
            this.react_coeff = diff_react_con.react_coeff;
            this.x = diff_react_con.x;
            this.coll_weights = diff_react_con.coll_weights;
            this.coll_points = diff_react_con.coll_points;
            this.nodes_to_coll_points = diff_react_con.nodes_to_coll_points;
            this.M = diff_react_con.M;
            this.S = diff_react_con.S;
        end
