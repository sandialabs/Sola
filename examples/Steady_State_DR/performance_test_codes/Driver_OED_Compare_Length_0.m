...
for p = 1:(N / p_seq)
    ...
    if p == 1
        covar_coeff = 1;
        delta_beta = abs(1 * ones(num_evals, 1));
        beta_0a = beta_0(1:end - num_evals);
    elseif p == 2
        covar_coeff = W_z_norm(z_bar - z_lofi)^2 / n;
        delta_beta = abs(beta_bar);
    else
    ...
    md_oed.Set_Covariance_Coefficient(covar_coeff);
    [beta_new, z_p] = md_oed.Generate_Seq_Optimal_Design_Con(beta_0a, alpha_d, 0, betas, beta_bar, delta_beta);
    betas = [betas; beta_new];
...
% hold on;
% plot([0 2:2:N], [Jhat_lofi; nonzeros(Jhat_DC_oed_2)] - subtract_qty, "r.-", "DisplayName", "$p=2$");
% plot([0 3:3:N], [Jhat_lofi; nonzeros(Jhat_DC_oed_3)] - subtract_qty, "g.-", "DisplayName", "$p=3$");
% plot([0 N], [Jhat_lofi; nonzeros(Jhat_DC_oed_6)] - subtract_qty, "k.-", "DisplayName", "$p=6$");
...
figure;
plot(0:N, [Jhat_lofi; Jhat_DC_oed_1], ".-", "Color", "#BAB86C", "DisplayName", "$p=1$");
hold on;
