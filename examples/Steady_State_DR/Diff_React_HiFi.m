classdef Diff_React_HiFi < Constrained_Optimization
    ...
    methods (Access = public)
        ...
        function [R,R_u] = Assemble_Reaction_Function(obj,u)
           ...
           x_nodes = obj.nodes_to_coll_points*obj.x;
           [R_nodes,R_prime_nodes] = obj.Reaction_Function(u_nodes,x_nodes);
        ...
        function [R_uu] = Assemble_Reaction_Function_Hessian(obj,u,lambda)
            ...
            x_nodes = obj.nodes_to_coll_points*obj.x;
            R_uu = obj.nodes_to_coll_points'*diag(obj.coll_weights)*obj.Reaction_Function_Hessian(u_nodes,lambda_nodes,x_nodes)*obj.nodes_to_coll_points;
        end
        
        function [R,R_prime] = Reaction_Function(obj,u,x)
           w = obj.Weight_Function(x);
           R = w.*(u.^2);
           R_prime = 2*diag(u.*w);
        end
        ...
        function [w] = Weight_Function(obj,x)
            w = 1+.7*sin(2*pi*x);
        end
    ...
    methods (Access = public)
        ...
        function obj = Diff_React_HiFi(diff_react)
            obj = obj@Constrained_Optimization();
            obj.diff_react = diff_react;
            obj.m = diff_react.m;
            obj.diff_coeff = diff_react.diff_coeff;
            obj.react_coeff = diff_react.react_coeff;
            obj.x = diff_react.x;
            obj.reg_coeff = diff_react.reg_coeff;
            obj.T = diff_react.T;    
            obj.coll_weights = diff_react.coll_weights;
            obj.coll_points = diff_react.coll_points;
            obj.nodes_to_coll_points = diff_react.nodes_to_coll_points;
            obj.M = diff_react.M;
            obj.S = diff_react.S;
            obj.reg_mat = diff_react.reg_mat;
        end
