...
opt_hifi = Hifi_Obj_Function(diff_react_hifi, obj);
...
plot(1, Jhat_oed_1, '.', 'MarkerSize', 3 * mark_size, 'color', colors(2, :));
legend({'Low-fidelity', 'One evaluation update'}, 'FontSize', 18);
for k = 1:p
    plot(N_range(k) - 0.1, Jhat_oed(k, :), 'o', 'MarkerSize', mark_size, 'color', colors(3, :), 'HandleVisibility', 'off');
    plot(N_range(k) - 0.1, mean(Jhat_oed(k, :)), 'd', 'MarkerSize', mark_size, 'color', colors(4, :), 'HandleVisibility', 'off');
    plot(N_range(k) + 0.1, Jhat_rand(k, :), 'x', 'MarkerSize', mark_size, 'color', colors(5, :), 'HandleVisibility', 'off');
    plot(N_range(k) + 0.1, mean(Jhat_rand(k, :)), 'd', 'MarkerSize', mark_size, 'color', colors(6, :), 'HandleVisibility', 'off');
end
