...
z_error_0 = Control_Norm(z_lofi - z_hifi, con_lofi) / normalization;
...
z_error_1 = Control_Norm(z_update_mean - z_hifi, con_lofi) / normalization;
...
for k = 1:p
    for i = 1:samps_per_N
        data_interface = MD_Data_Interface_Adv_Diff(k, i, 'OED');
        data_interface.Load_Data();
        md_update = MD_Update(opt_prob_interface, data_interface, u_prior_interface, z_prior_interface, md_hessian_analysis);
        md_update.Compute_Posterior_Data(alpha_d, num_post_samps);
        [z_update_mean, z_update_samples] = md_update.Posterior_Update_Samples();
        Jhat_oed(k, i) = opt_hifi.Jhat(z_update_mean);
        oed_z_error(k, i) = Control_Norm(z_update_mean - z_hifi, con_lofi) / normalization;

        data_interface = MD_Data_Interface_Adv_Diff(k, i, 'Random');
        data_interface.Load_Data();
        md_update = MD_Update(opt_prob_interface, data_interface, u_prior_interface, z_prior_interface, md_hessian_analysis);
        md_update.Compute_Posterior_Data(alpha_d, num_post_samps);
        [z_update_mean, z_update_samples] = md_update.Posterior_Update_Samples();
        Jhat_rand(k, i) = opt_hifi.Jhat(z_update_mean);
        rand_z_error(k, i) = Control_Norm(z_update_mean - z_hifi, con_lofi) / normalization;
    end
end
