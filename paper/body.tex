...
On the other hand, if $\bftheta$ can be chosen so that $\tilde{\s}(\z) + \bfdelta(\z;\bftheta) \approx \s(\z)$ for all $\z$, then
$
    \J(\z,\bftheta)
...
\subsection{Post-optimality sensitivity analysis}
...
Let $\tilde{\z}\in\R^{n_z}$ be a local minimizer of~\cref{eq:optimization-surrogate}.
...
The right-hand side of \cref{eq:control-update} has three components: the solution $\tilde{\z}$, which can computed by applying the adjoint method to \cref{eq:optimization-surrogate};
...
\SM{$\bfdelta$ is affine in $\z$ and linear in $\bftheta$}

\begin{purpose}
    Applying the update is hard. View the update as a Bayesian problem, then we need a prior for $\bftheta$, which will be written in terms of priors for $\u$ and $\z$.
\end{purpose}
