...
\\ % \hline
3. & Application of state mass matrix, $\v\mapsto\M_u\v$ % Apply_M_u
\\
4. & \SM{What to call this?}, $\v\mapsto(\W_u + \alpha\M_u)^{-1}\v$ % Apply_W_u_Plus_scalar_M_u_Inverse
\\
5. & Application of state covariance matrix, $\v\mapsto\W_u^{-1}\v$ % Apply_W_u_Inverse
% Sample_with_Covariance_W_u_Inverse
% Sample_with_Covariance_W_u_Plus_scalar_M_u_Inverse
% & \texttt{MD\_u\_Prior\_Interface}
\\ % \hline
6. & Application of control covariance matrix, $\v\mapsto\W_z^{-1}\v$ % Apply_W_z_Inverse
% Sample_with_Covariance_W_z_Inverse
% Apply_W_z
% & \texttt{MD\_z\_Prior\_Interface}
\\ % \hline
7. & Application of surrogate constraint Jacobian transpose, $\v\to\tilde{\s}_{z}(\z)\trp\v$ % Apply_Solution_Operator_z_Jacobian_Transpose
\\
8. & Application of surrogate reduced-space Hessian, $\v\mapsto\tilde{J}_{z,z}(\tilde{\z})\v$ % Apply_RS_Hessian
\\
9. & Objective state gradient, $\grad{u}J(\u,\z)$ % Misfit_Gradient
\\
10. & Application of objective state Hessian, $\v\mapsto\grad{u,u}J(\u,\z)\v$ % Apply_Misfit_Hessian
...
To that end, this section presents \SABL: \textbf{S}andbox for \textbf{A}adjoint-\textbf{B}ased outer \textbf{L}oop analysis, a \MATLAB library that implements HDSA for model discrepancy.
