...
Defining $\u \coloneqq \s(\z)$, the first term in \cref{eq:gradJ-chain-rule} becomes
\begin{align}
    \begin{aligned}
...
\begin{purpose}
High-level overview of how $\bftheta$ is calibrated and the model update is done.
...
To address the model discrepancy error in \cref{eq:optimization-surrogate}, let $\bfdelta \coloneqq \bfdelta(\z;\bftheta)\in\R^{n_z}$ for some parameter vector $\bftheta\in\R^{n_\theta}$ and consider the perturbed objective function
\begin{align}
    \label{eq:optimization-discrepancy}
    \J(\z, \bftheta)
    \coloneqq J\big(\tilde{\s}(\z) + \bfdelta(\z;\bftheta), \z\big).
\end{align}
If $\bftheta$ can be chosen so that $\bfdelta(\z;\bftheta) \approx \s(\z) - \tilde{\s}(\z)$ for all $\z$, then $\tilde{\s}(\z) + \bfdelta(\z;\bftheta) \approx \s(\z)$ and
$
    \J(\z,\bftheta)
    ...
    % = \hat{J}(\z).
$
\SM{Implicit function theorem on $\grad{z}\J$. Why is the update $\tilde{\z} + \grad{\theta}F(\0)\bftheta$? Applying the update is hard. View the update as a Bayesian problem, then we need a prior for $\bftheta$, which will be written in terms of priors for $\u$ and $\z$.}
