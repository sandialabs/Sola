...
We can then use \SABL as a guide for developing a low-level HPC implementation of HDSA-MD in \cpp, which is the topic of \Cref{sec:hpc}.
...
\noindent
\SABL is written with the object-oriented \MATLAB protocol, which translates easily to \cpp.
...
To solve a particular problem, the user defines child classes to implement the abstract methods, then uses pre-defined tools to put the pieces together and execute the algorithm.

Our first design task is to identify the pieces of the HDSA-MD workflow that are problem dependent.
These include derivatives of $J$, $\tilde{J}$, and $\tilde{\s}$; mass and precision matrices for the state and control spaces; and data for the Bayesian inference \cref{eq:bayesian-inference}.
...
Requirements 8--10 arise from an efficient sampling of the posterior parameter distribution, which we will discuss shortly.
...
Sampling from the prior distribution for $\bftheta$, applying $\B$ and $\H^{-1}$, sampling from the posterior distribution~\cref{eq:posterior-parameter-distribution}, followed by the control update \cref{eq:control-update}, requires the ingredients listed in \Cref{table:hdsa-requirements}.
...
\SM{Naming / argument conventions? What else do we talk about here? FEM verifies with method of manufactured solutions. Comparing code to code is not verification, but that's not verification because the first code might be wrong, but we have no other choice. There are things we can do to verify: check for symmetry / positive definiteness, checkpointing quantities throughour algorithmic implementation, etc. We can also try to set up inverse problems for a type of method of manufactured solutions. All of these tasks would be much harder in HPC, but \MATLAB just does it (example: \texttt{eig()} to determine positive definiteness). Maybe focus a bit on how we can verify things (gradients, positive definiteness, symmetry, proptotypical examples to demonstrated improvements in the solution.)}
...
Finite difference methods, unit tests, method of manufactured solutions (forward problem, optimization, inverse problems), etc.
