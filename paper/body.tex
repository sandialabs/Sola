...
Although solving~\cref{eq:optimization-reducedspace} directly is intractable due to the derivatives of the full constraint $\c$ being inaccessible or highly expensive, we assume that we can either (1) solve the constraint equation (evaluate $\s(\z)$) a limited number of times or (2) in a data-driven scenario, observe a limited number of state solutions.
To that end, for a linearly independent set of controls $\{\z_{\ell}\}_{\ell=1}^{n_d}$ for which we have access to $\s(\z_1),\ldots,\s(\z_{n_d})$ and $\tilde{\s}(\z_1),\ldots,\tilde{\s}(\z_{n_d})$ and consider the Bayesian linear inference problem
\begin{align}
    \left[\begin{array}{cc}
    ...
    + \bfepsilon,
\end{align}
where $\bfepsilon\sim\mathcal{N}(\0, \alpha\M_u^{-1})$ is an uncorrelated Gaussian noise term with variance $\alpha > 0$ and in which $\M_u\in\R^{n_u\times n_u}$ is a mass matrix for the state space.
{\color{royalblue}
...
In the context of PDEs, these precision matrices are often (inverse?) elliptic operators.
It can be shown that an appropriate precision matrix for the parameters $\bftheta$ is
\begin{align}
    \W_\theta
    = \left[\begin{array}{cc}
    ...
    \in\R^{p\times p}.
\end{align}
Posterior is also Bayesian, also uses $\M_u$ matrix-vector products due to the noise model (??)

At this point we're pretty much done, lots of previous work has gone into showing how to do all of the computations efficiently with randomized linear algebra, matrix factorizations, and tuning hyperparameters for the priors.
Citations.
}
