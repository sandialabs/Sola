...
An \texttt{Objective} class is defined for the objective $J$ (or $\tilde{J}$) and a separate \texttt{Constraint} class is defined to represent the constraint $\c$ (or $\tilde{\c}$).
Each contains abstract (pure virtual) methods that represent the required evaluations and derivatives, as well as provided methods that compare the user-defined derivatives to finite difference estimates with varying step sizes.
Class outlines are given in \Cref{code:sabl-optimization}.

\SM{ReducedSpaceOptimization space optimization. Naming / argument conventions?}
...
\subsection{HDSA implementation}
