...
If $\c(\u,\z)=\0$ admits a unique solution for any given $\z$ --- that is, if the discretized PDE is well posed at the given control --- then there exists a solution operator $\S:\R^{n_z}\to\R^{n_u}$ such that $\c(\S(\z),\z)=\0$ for all possible controls $\z$.
...
Using $\c(\S(\z),\z) = \0$, the implicit function theorem gives the formula
\begin{align}
    \grad{z}\hat{J}(\z)
    = \nabla[J(\S(\z),\z)]
