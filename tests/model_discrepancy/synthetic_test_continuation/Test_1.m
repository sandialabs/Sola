...
rel_diffs = [];
rel_diffs(end+1) = rel_err(u_cont, ref_sol.u_cont);
rel_diffs(end+1) = rel_err(z_cont, ref_sol.z_cont);
rel_diffs(end+1) = rel_err(beta_cont, ref_sol.beta_cont);
% rel_diffs(end+1) = rel_err(u_ks, ref_sol.u_ks);
% rel_diffs(end+1) = rel_err(z_ks, ref_sol.z_ks);
% rel_diffs(end+1) = rel_err(beta_ks, ref_sol.beta_ks);

if max(rel_diffs) > 1.e-9
