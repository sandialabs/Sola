...
alpha_u = 1.e-2;
alpha_z = 1.e-12;
u_prior_interface = MD_Elliptic_u_Prior_Interface_Transient_Test_Problem(alpha_u, transient_prior_cov, opt);
...
test = sqrt(alpha_u) * kron(transient_prior_cov.E_td_inv_evecs * diag(sqrt(transient_prior_cov.evals)), u_prior_interface.sing_vecs_output * diag(u_prior_interface.sing_vals)) * u_in;
...
aleph = kron(transient_prior_cov.evals, u_prior_interface.sing_vals.^2);
aleph = aleph ./ (1 + alpha_u * scalar * aleph);
test  = sqrt(alpha_u) * kron(transient_prior_cov.E_td_inv_evecs, u_prior_interface.sing_vecs_output) * diag(sqrt(aleph)) * u_in;
