...
To implement the abstract methods of {class}`Objective` for this particular choice of $J$, we need to calculate the derivatives of $J$.
...
We can now begin with the {class}`Optimize` [template](optimization.Objective.template), add a constructor for setting the constants $\alpha_{1},\ldots,\alpha_{5}$, and implement the abstract methods.
...
:::{tip}
Even though we can easily write out the Hessians of $J$ in this case, our implementation avoids explicitly forming $\grad{u,u}J$ and its siblings.
...
With these derivatives calculated, we may now define a {class}`Constraint` subclass that implements the abstract methods by filling out the {class}`Constraint` [template](optimization.Constraint.template).
...
We can now instantiate our `Example_1_Objective` and `Example_1_Constraint` classes and couple them with a {class}`Reduced_Space_Optimization` object.
...
The {class}`Objective` and {class}`Reduced_Space_Optimization` classes both have these verification methods.
