...
:::{danger}
Because of how MATLAB's [`fminunc()`](https://www.mathworks.com/help/optim/ug/fminunc.html) is designed, the methods listed above (except for `J()`) must be implemented in a _vectorized_ fashion, i.e., assuming that `u_in`/`z_in` is a matrix where each column is a test direction.
:::
