...
\begin{tabular}{|l|l|}
    ...
    Function Signature & Mathematical Description
    \\ \hline
    \texttt{[val,grad\_u] = Time\_Instance\_Objective(u,t)}
    & Evaluate $g(u,t)$ and $\nabla_u g(u,t)$
    \\
    \texttt{[val,grad\_z] = Regularization\_Objective(z)}
    & Evaluate $R(z)$ and $\nabla_z R(z)$
    \\
    \texttt{[f,f\_u,f\_z] = Time\_Instance\_RHS(u,z,t)}
    & Evaluate $f(u,z,t)$, $f_u(u,z,t)$, and $f_z(u,z,t)$
    \\
    \texttt{[h, h\_z] = Initial\_Condition(z)}
    & Evaluate $h(z)$ and $h_z(z)$
    \\
    \texttt{[Mv] = Time\_Instance\_Objective\_uu\_Apply(v,u,t)}
    & Compute the product $\nabla_{u,u} g(u,t) v$
    \\
    \texttt{[Mv] = Regularization\_Objective\_zz\_Apply(v,z)}
    & Compute the product $\nabla_{z,z} R(z) v$
    \\
    \texttt{[Mv] = Time\_Instance\_RHS\_uu\_Apply(v,u,z,t,lambda)}
    & Compute the product $\lambda\trp f_{u,u}(u,z,t) v$
    \\
    \texttt{[Mv]  = Time\_Instance\_RHS\_uz\_Apply(v,u,z,t,lambda)}
    & Compute the product $\lambda\trp f_{u,z}(u,z,t) v$
    \\
    \texttt{[Mv]  = Time\_Instance\_RHS\_zu\_Apply(v,u,z,t,lambda)}
    & Compute the product $\lambda\trp f_{z,u}(u,z,t) v$
    \\
    \texttt{[Mv]  = Time\_Instance\_RHS\_zz\_Apply(v,u,z,t,lambda)}
