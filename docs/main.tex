...
% As we present the mathematics used in \texttt{Constrained\_Optimization} we introduce functions, properties, and inherited classes corresponding to the mathematical expressions.
To facilitate our analysis, we require efficient algorithms to compute the gradient $\nabla_z \hat{J}(z)\in\R^{n_{z}}$ and Hessian-vector products $\nabla_{z,z} \hat{J}(z) v$ for a given vector $v\in\R^{n_{z}}$ without explicitly forming the Hessian $\nabla_{z,z}\hat{J}(z)\in\R^{n_{z}\times n_{z}}$. To this end we introduce adjoint-based derivatives which is implemented in the code. Algorithm~\ref{alg:adjoint_gradient} demonstrates how to efficiently calculate the gradient. Throughout this document we use $c_u(u,z)\in\R^{n_{c}\times n_{u}}$ and $c_z(u,z)\in\R^{n_{c}\times n_{z}}$ to denote the Jacobians of $c$ with respect to $u$ and $z$, respectively, $\nabla_u J(u, z)\in\R^{n_{u}}$ and $\nabla_z J(u, z)\in\R^{n_{z}}$ to denote the gradients (where $u$ and $z$ are independent of one another) of $J$ with respect to $u$ and $z$, respectively, and a superscript $\mathsf{T}$ to denote a matrix transpose. Based on Algorithm~\ref{alg:adjoint_gradient}, Table~\ref{tab:con_opt_pure_virtual_funs_1} lists the pure virtual functions defined in \texttt{Constrained\_Optimization} which must be implemented for a given problem to enable the gradient computation.
...
\begin{tabular}{|l|l|}
    ...
    \\ \hline
    \texttt{[val,grad\_u,grad\_z] = Objective(u,z)}
    & Evaluate $J$, $\nabla_u J$, and $\nabla_z J$
    \\
    ...
    \\ \hline
\end{tabular}
\caption{Pure virtual function in \texttt{Constrained\_Optimization} needed to execute Algorithm~\ref{alg:adjoint_gradient}.}
\label{tab:con_opt_pure_virtual_funs_1}
\end{table}

To compute efficient Hessian-vector products, Algorithm~\ref{alg:adjoint_hessvec} outlines computation using incremental state and incremental adjoint equations. We use $\lambda\trp c_{u,u}(u,z) \in \R^{n_{u}\times n_{u}}$ and $\nabla_{u,u} J(u,z)\in\R^{n_{u}\times n_{u}}$ to denote the $(u,u)$ Hessian of the scalar functions $\lambda\trp c$ and $J$, respectively, and similar expressions for the $(u,z)$, $(z,u)$, and $(z,z)$ Hessians. Table~\ref{tab:con_opt_pure_virtual_funs_2} lists the pure virtual functions in \texttt{Constrained\_Optimization} which must be implemented for a given problem to enable Hessian-vector product computations using Algorithm~\ref{alg:adjoint_hessvec}.
...
\begin{tabular}{|l|l|}
    ...
    \\
    \texttt{[Mv] = Objective\_uu\_Apply(v,u,z)}
    & Compute the product $\nabla_{u,u} J(u,z) v$
    \\
    \texttt{[Mv] = Objective\_uz\_Apply(v,u,z)}
    & Compute the product $\nabla_{u,z} J(u,z) v$
    \\
    \texttt{[Mv] = Objective\_zu\_Apply(v,u,z)}
    & Compute the product $\nabla_{z,u} J(u,z) v$
    \\
    \texttt{[Mv] = Objective\_zz\_Apply(v,u,z)}
    & Compute the product $\nabla_{z,z} J(u,z) v$
    \\ \hline
\end{tabular}
\caption{Pure virtual function in \texttt{Constrained\_Optimization} needed to execute Algorithm~\ref{alg:adjoint_hessvec}.}
...
\end{array} \right)
\end{align*}
and compute $x=c_u(u,z)\invtrp v$ via the backwared-in-time stepping algorithm
\begin{align*}
