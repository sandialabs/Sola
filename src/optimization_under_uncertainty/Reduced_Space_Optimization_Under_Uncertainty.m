...
classdef Reduced_Space_Optimization_Under_Uncertainty < handle
    ...
    %  min_{u_1,u_2,\dots,u_N,z} \frac{1}{N} \sum_{i=1}^N J(u_i,z)
    %  subject to
    %  c(u_i,z,theta_i) = \0
    % by solving the equivalent, unconstrained reduced-space problem
    % min_z \hat{J}(z) := \frac{1}{N} \sum_{i=1}^N J(S_i(z), z)
    % where \S_i(z) solves the constraint equation c(u_i,z,theta_i) = 0
