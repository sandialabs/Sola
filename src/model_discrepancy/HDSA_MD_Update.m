classdef HDSA_MD_Update < handle
    ...
    methods
        ...
        function this = HDSA_MD_Update(md_interface)
        ...
        function [] = Compute_Posterior_Data(this, alpha_d, num_samples)
            this.post_data.Compute_Posterior_Data(this.md_interface, alpha_d, this.u_opt, this.z_opt, num_samples);
        end

        function [] = Compute_Hessian_GEVP(this, num_evals, oversampling)
            this.gevp = Hessian_GEVP(this.md_interface, this.z_opt);
            this.gevp.Compute_Hessian_GEVP(num_evals, oversampling);
        end
        ...
        function [delta_mean, delta_samples] = Posterior_Discrepancy_Samples(this, z)
            ...
            for k = 1:p
                ...
                W_z_Inv_dz = this.md_interface.Apply_W_z_Inverse(dz);
        ...
        function [z_update_mean, z_update_samples] = Posterior_Update_Samples(this)
            ...
            tmp = this.md_interface.Apply_Misfit_Hessian(u_tmp1, this.u_opt, this.z_opt);
            Btheta_hat = Btheta_hat + this.md_interface.Apply_Solution_Operator_z_Jacobian_Transpose(tmp, this.z_opt);
            ...
            if ~isempty(this.gevp)
                z_update_samples = z_update_mean -   this.gevp.Apply_Projected_RS_Hessian_Inverse(Btheta_hat + Btheta_breve);
            else
                z_update_samples = z_update_mean - this.md_interface.Apply_RS_Hessian_Inverse(Btheta_hat + Btheta_breve, this.z_opt);
            end
        end
        ...
        function [z_update_mean] = Posterior_Update_Mean(this)
            ...
            tmp1 = this.md_interface.Apply_Misfit_Hessian(u, this.u_opt, this.z_opt);
            z_tmp = this.md_interface.Apply_Solution_Operator_z_Jacobian_Transpose(tmp1, this.z_opt);
            ...
            if ~isempty(this.gevp)
                z_pert = -this.gevp.Apply_Projected_RS_Hessian_Inverse(z_tmp);
            else
                z_pert = -this.md_interface.Apply_RS_Hessian_Inverse(z_tmp, this.z_opt);
            end
